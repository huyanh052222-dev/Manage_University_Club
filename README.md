...
Doanh thu tối đa của 10 đơn thường tăng theo uy tín: 1 sao = 200 coin, 2 sao = 300 coin, 3 sao = 400 coin, 4 sao = 500 coin và 5 sao = 600 coin. Quỹ này chia đều cho 10 đơn, tương ứng 20/30/40/50/60 coin mỗi đơn. Đơn đặc biệt luôn hiển thị mức `Từ 200-1000 coin` và tạm ẩn hạn ở cả thẻ danh sách lẫn phần chi tiết. Nút nguồn dùng liên hệ Zalo riêng theo quán từ `ORDER_CONTACTS_BY_TEAM`; liên hệ cũ của NPC Hiếu (`0703500256`) tại Fortuna đã được thay bằng liên kết giải mã từ QR `http://zaloapp.com/qr/p/t22cf13d4l8c`. Demo đánh dấu đơn đầu tiên là “Đơn đặc biệt”, làm nổi bật và luôn đưa đơn đó lên đầu danh sách. Chức năng tiến độ đang được tạm bỏ và danh sách đơn có vùng cuộn riêng để không kéo dài dashboard.

`teams.reputation` lưu uy tín quán theo thang 1–5 sao. Mọi quán bắt đầu ở mức 1 sao; giao diện luôn render đủ năm ngôi sao và chỉ tô sáng số sao tương ứng. Admin có mục **Quản lý sao** để nâng/hạ từng quán qua RPC `update_team_reputation`; thay đổi sẽ tự cập nhật mức thưởng đơn thường ở lần tải trang kế tiếp.

`coin_transactions` là sổ cái biến động coin. Ba ô tổng coin vào/ra/thay đổi ròng bên dưới Nhật ký tổng hợp các giao dịch đã tải từ sổ cái; riêng “Doanh thu tuần” chỉ cộng các giao dịch loại `income` từ 00:00 Thứ Hai đến trước 00:00 Thứ Hai kế tiếp theo múi giờ `Asia/Ho_Chi_Minh`, tức bao gồm trọn Chủ nhật. Mỗi lần Admin cộng coin dương được ghi là doanh thu của tuần tại thời điểm cộng, xuất hiện trong nhật ký và được đưa vào kỳ kết toán; thao tác trừ thủ công vẫn là `adjustment`, không làm giảm doanh thu. Admin bắt buộc nhập lý do (tối đa 200 ký tự) cho mỗi lần cộng hoặc trừ thủ công; lý do được lưu riêng trong `coin_transactions.reason` và hiển thị trong Nhật ký coin của đúng quán. Khi tuần mới chưa được Admin cộng coin, doanh thu tuần hiển thị `0 coin`. RPC `add_points_to_team` cập nhật `teams.points` và ghi cả lý do vào nhật ký trong cùng một giao dịch SQL.
...
Với database đang hoạt động, chạy thêm `scripts/supabase/admin_reputation.sql` đúng một lần trước khi dùng mục **Quản lý sao**. Script tạo RPC chỉ dành cho tài khoản có `app_metadata.role = admin`; người dùng anon không thể gọi hàm nâng/hạ sao.

...
Doanh thu tối đa của 10 đơn thường tăng theo uy tín: 1 sao = 200 coin, 2 sao = 300 coin, 3 sao = 400 coin, 4 sao = 500 coin và 5 sao = 600 coin. Quỹ này chia đều cho 10 đơn, tương ứng 20/30/40/50/60 coin mỗi đơn. Đơn đặc biệt luôn hiển thị mức `Từ 200-800 coin` và tạm ẩn hạn ở cả thẻ danh sách lẫn phần chi tiết. Nút nguồn của đơn đặc biệt dùng liên hệ Zalo riêng theo quán từ `ORDER_CONTACTS_BY_TEAM`; liên hệ cũ của NPC Hiếu (`0703500256`) tại Fortuna đã được thay bằng liên kết giải mã từ QR `http://zaloapp.com/qr/p/t22cf13d4l8c`. Đối với các đơn thường còn lại, nút nguồn đơn hàng chuyển hướng đến trang bài tập `https://judge.bcn.id.vn/mentor/bai-tap`. Demo đánh dấu đơn đầu tiên là “Đơn đặc biệt”, làm nổi bật và luôn đưa đơn đó lên đầu danh sách. Chức năng tiến độ đang được tạm bỏ và danh sách đơn có vùng cuộn riêng để không kéo dài dashboard.

Uy tín quán dùng thang 1–5 sao. Giao diện luôn render đủ năm ngôi sao và chỉ tô sáng số sao tương ứng. Admin có mục **Quản lý sao** để nâng/hạ từng quán; thao tác gọi RPC `set_team_reputation` và lưu vào `public.teams.reputation` trên Supabase, vì vậy mọi thiết bị sẽ thấy cùng một mức sao sau khi tải lại trang. Chạy [`scripts/supabase/reputation_sync.sql`](scripts/supabase/reputation_sync.sql) một lần trước khi dùng chức năng này. Chi tiết nằm trong `HUONG_DAN_DEMO_SAO.txt`.

`coin_transactions` là sổ cái biến động coin. Ba ô tổng coin vào/ra/thay đổi ròng bên dưới Nhật ký tổng hợp các giao dịch đã tải từ sổ cái và hiển thị số lần cộng/trừ. Mỗi dòng có lý do: Admin bắt buộc nhập tối đa 200 ký tự khi cộng/trừ thủ công; còn chi phí vận hành, vốn và dữ liệu khôi phục có lý do do hệ thống tự ghi. Form **Cộng coin** có lựa chọn theo tuần: mặc định là tuần đang diễn ra; cũng có thể chọn Tuần 1, Tuần 2, Tuần 3… đã hoàn tất. Khi chọn kỳ cũ, coin chỉ đổi số dư một lần và giao dịch được gắn `settlement_period_start` cho kỳ đó. Khoản cộng được tính là doanh thu; khoản trừ thủ công được tính là **chi phí bổ sung** của kỳ, ngoài chi phí vận hành cố định. Chạy `scripts/supabase/manual_deduction_settlement.sql` để áp dụng quy tắc này và tính lại các kỳ đã chốt trước đó. Khi tuần mới chưa được Admin cộng coin, doanh thu tuần hiển thị `0 coin`.
...
Mục Admin **Chốt lại doanh thu kỳ trễ** liệt kê các kỳ Thứ Hai–Chủ nhật đã hoàn tất và trạng thái số quán đã chốt. Trong tuần, mọi lần Admin cộng coin dương qua luồng thông thường vừa tăng `teams.points` vừa ghi `income`; khi đầu tuần sau chốt kỳ, các dòng này được tính tự động vào doanh thu. Nếu kỳ đã qua còn **Chờ kết toán** hoặc đã có giao dịch cộng muộn, Admin chọn quán/kỳ và dùng **Chốt lại theo nhật ký hiện có** hoặc **Gán coin đã chọn vào kỳ**. Hai nút ở mục kỳ trễ chỉ cập nhật `weekly_financial_settlements` (và trường `settlement_period_start` của giao dịch đã tồn tại), không tạo giao dịch mới, không sửa `teams.points`. Riêng form **Cộng coin** ở Bảng xếp hạng có thể chủ động thêm coin mới cho kỳ cũ; thao tác đó tăng số dư một lần và gắn kỳ ngay khi ghi sổ.
...
Để bật quyền chốt/gán doanh thu cho kỳ trễ và chọn kỳ ngay khi cộng coin, chạy `scripts/supabase/late_settlement_adjustments.sql` đúng một lần. Migration thêm trường gán kỳ vào sổ cái, sửa hàm chốt tuần để không đếm trùng khoản đã gán kỳ cũ, và thêm RPC cộng coin cho một kỳ đã hoàn tất. Chạy migration không tự cộng coin, không tự tạo giao dịch và không thay đổi `teams.points`.

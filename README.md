...
Uy tín quán dùng thang 1–5 sao. Giao diện luôn render đủ năm ngôi sao và chỉ tô sáng số sao tương ứng. Admin có mục **Quản lý sao** để nâng/hạ từng quán; giá trị được lưu trong `localStorage` với khóa `cafe-reputation-overrides-v1` và tự cập nhật mức thưởng đơn thường. Cách này không cần RPC hay migration database, nhưng chỉ có hiệu lực trên cùng trình duyệt và cùng domain; xóa dữ liệu trình duyệt hoặc mở trên thiết bị khác sẽ dùng lại mức mặc định từ `teams.reputation`.

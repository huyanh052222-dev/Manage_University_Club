...
Mục Admin **Chốt lại doanh thu kỳ trễ** liệt kê các kỳ Thứ Hai–Chủ nhật đã hoàn tất và trạng thái số quán đã chốt. Trong tuần, mọi lần Admin cộng coin dương qua luồng thông thường vừa tăng `teams.points` vừa ghi `income`; khi đầu tuần sau chốt kỳ, các dòng này được tính tự động vào doanh thu. Nếu kỳ đã qua còn **Chờ kết toán** hoặc coin đã được cộng muộn sau khi kỳ kết thúc, Admin chọn quán/kỳ và dùng **Chốt lại theo nhật ký hiện có** hoặc **Gán coin đã chọn vào kỳ**. Cả hai chỉ cập nhật `weekly_financial_settlements` (và trường `settlement_period_start` của giao dịch đã tồn tại), không tạo giao dịch mới, không sửa `teams.points`; nhờ vậy không thể cộng trùng số dư.
...
Để bật quyền chốt/gán doanh thu cho kỳ trễ, chạy `scripts/supabase/late_settlement_adjustments.sql` đúng một lần. Migration thêm trường gán kỳ vào sổ cái và các RPC chỉ đọc nhật ký hiện có để cập nhật kết toán; migration không cộng coin, không tạo giao dịch mới và không thay đổi `teams.points`.

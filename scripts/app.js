...
const handleAction = (actionElement) => {
    ...
    if (action === "show-profile") {
        showModal({
            ...
            content:
                '<p><strong>Quản trị viên · Admin hệ thống</strong></p><p>Đây là tài khoản đăng nhập duy nhất, có quyền điều chỉnh coin của các nhóm. Thành viên chỉ được lưu dưới dạng hồ sơ hiển thị.</p><div class="account-modal-actions"><button class="secondary-button logout-button" type="button" data-action="logout">Đăng xuất</button></div>',
        });
        return;
    }
